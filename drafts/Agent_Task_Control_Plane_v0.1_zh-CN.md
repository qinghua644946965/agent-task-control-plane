...
## 3.7 优选基础能力：原生有状态任务续接

ATCP 可以通过外部维护的检查点、摘要、事件、产物和上下文重建运行，但其优选基础能力是 [Native Stateful Task Continuation（原生有状态任务续接）](https://github.com/qinghua644946965/native-stateful-task-continuation)：由用户授权的控制平面能够稳定寻址并继续已有的厂商原生任务，而不是把它重新创建为一次新的模型调用。

两个层次承担不同职责：

- **ATCP 管理全局状态：**目标、任务关系、路由、生命周期迁移、审批、验证与追踪。
- **厂商维护局部任务状态：**原生上下文、执行历史、可恢复的文件与工具状态、摘要和检查点。

这种分工在保持用户掌握全局治理权的同时，也保持了 **State Locality（状态局部性）**。

ATCP 定义三个能力层级：

1. **无状态重建**——调用方在每次调用时重新构造所需上下文。
2. **外部检查点恢复**——控制平面恢复自己维护的摘要、状态、事件和产物。
3. **原生有状态续接**——控制平面稳定寻址并继续原来的厂商原生任务。

第三层是优选运行形态，但不是硬性依赖。ATCP 在第一、第二层仍然有价值，因此可以在原生续接能力逐步成熟的过程中渐进采用。
